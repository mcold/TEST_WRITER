; сохранени€ разницы времени A_Now в глобальной переменной
; сохранени€ скрипта файлом
...
KeysCount := 0
Rec := 1 ; флаг записи
t_prev := A_Now						; предыдущее врем€
t_Time := A_Now						; текущее врем€


global Macro2 := {}					; словарь дл€ сохранени€ последовательности текста
global Array := Object()
...
^LButton::
	...
	; получаем разницу во времени выполнени€
	TimeRewrite()
	delta := (t_Time - t_prev) * 1000
	
	; формируем запись действи€ пользовател€
...
Add(Key) ; запоминание нажатой клавиши
{
    ...
    If Rec = 1
    {
		...
		TimeRewrite()					; переписываем врем€
    }
}

InsertSymbols()				; функци€ записывает текст в массив
{
	n = 0					; по переменной определ€ем наличие текста	
	global KeysCount
	symbols := 
	Loop %KeysCount%
	{
...
F2::
; сохранение скрипта
FileSelectFile, SelectedFile, 8, ,ќткрыть файл, “екстовые файлы (*.ahk) 					; выбор файла с созданием
for i in Array
{
	if % Array[i].type = "button"
	{
...
IfWinExist, %tt%
    WinActivate ; »спользуетс€ окно, найденное выше
else
    WinActivate, %tt%
return
